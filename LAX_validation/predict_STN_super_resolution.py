...
for j in range(0,len(model_files)):
  ...
  for i in range(0, x_list.shape[0]):
    ...
    xx = ff.find_all_target_files(['pred_img_LR*'], os.path.join(save_folder, patient_id))
    for kx in range(0,len(xx)):
      x = xx[kx]
      ...
      # save image
      nb.save(nb.Nifti1Image(final_pred.astype(float), u_gt_nii.affine), os.path.join(folder, 'pred_img_HR_' + str(len(xx) * j + kx + 1) +'.nii.gz'))
