...
for jj in range(0,len(center_x_files)):
    for j in range(0,len(files)): #center_x: j=0, center_y: j=1,
        ...
        for i in range(0,patient_id_list.shape[0]):
            ...
            # use a random y file
            y = os.path.join(cg.data_dir,'simulated_data_version2/155/ED/ds/centerlist.npy')

            datagen = Generator_motion.DataGenerator(np.asarray([input_list[i]]), np.asarray([y]),
                                            patient_num =1,
