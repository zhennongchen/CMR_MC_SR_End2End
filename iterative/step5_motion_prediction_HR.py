...
for jj in range(0,len(center_x_files)):
    for j in range(0,len(files)): #center_x: j=0, center_y: j=1,
        ...
        for i in range(0,1200):#x_list_predict.shape[0]):
            ...
            # in training?
            allow_batches = mm.Motion_ResNet_HR_index(batch)
     
            if jj not in allow_batches:
                continue

            # have image data?
            if os.path.isfile(x_list_predict[i]) == 0 or os.path.isfile(y_list_predict[i]) == 0:
            ...
            datagen = Generator_motion.DataGenerator(np.asarray([x_list_predict[i]]), np.asarray([y_list_predict[i]]),patient_num =1,batch_size = cg.batch_size, 
                                            num_classes = 2,
