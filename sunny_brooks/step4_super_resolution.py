...
for i in range(0,patient_id_list.shape[0]):
    ...
    final_pred_flip= final_pred[:,:,[final_pred.shape[-1] - i for i in range(1,final_pred.shape[-1] + 1)]]
    pred_nb_flip = nb.Nifti1Image(final_pred_flip, nb.load(y).affine)
    nb.save(pred_nb_flip, os.path.join(pred_path, 'pred_img_HR_' + object +'_flip.nii.gz'))
...
#     if os.path.isfile(os.path.join(pred_path, 'pred_img_HR_LV.nii.gz')) == 0 or os.path.isfile(os.path.join(pred_path, 'pred_img_HR_Myo.nii.gz')) == 0:
...
#     lvmyo = nb.load(os.path.join(pred_path, 'pred_img_HR_Myo.nii.gz')).get_data()
#     lv_myo = np.round(lvmyo); lv_myo = lv_myo.astype(int)

#     lvmyo[lvmyo == 1] = 2
#     lvmyo[lv==1] = 1

#     nb.save(nb.Nifti1Image(lvmyo, affine, header), os.path.join(pred_path, 'pred_img_HR_LVmyo.nii.gz'))
