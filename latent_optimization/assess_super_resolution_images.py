...
for i in range(0,x_list_predict.shape[0]):
    ...
    if os.path.isfile(os.path.join(pred_path,'predict.nii.gz')) == 0:
        print('not done; skip'); continue

    # load pred 
    pred = nb.load(os.path.join(pred_path,'predict.nii.gz')).get_fdata()
    ...
    ##### Calculate DICE
    # move for best Dice
    if os.path.isfile(os.path.join(pred_path, 'move_for_dice.npy')) == 0:
        max_dice, ds, pred_for_dice = util.Dice_optim(pred, gt, [1,2], d = 5)
        np.save(os.path.join(pred_path, 'move_for_dice.npy'), np.asarray(ds))
    else:
        ds = np.load(os.path.join(pred_path, 'move_for_dice.npy'), allow_pickle = True)
    ...
    # move for HD
    if os.path.isfile(os.path.join(pred_path,'move_for_HD.npy')) == 0:
        _, ds2, pred_for_HD = util.HD_optim(pred, gt, min  = False, k_list = [1,2], d = 5)
        np.save(os.path.join(pred_path, 'move_for_HD.npy'), np.asarray(ds2))
    else:
        ds2 = np.load(os.path.join(pred_path,'move_for_HD.npy'), allow_pickle = True)
    ...
    df.to_excel(os.path.join(os.path.dirname(save_folder), 'comparison_latent_optimization_test.xlsx'), index = False)
