...
for j in range(1,len(model_files)):
  f = model_files[j]
  print(f)
  model = Model(inputs = model_inputs,outputs = model_outputs)
  ...
  for i in range(0, x_list.shape[0]):
    ...
    # x = os.path.join(cg.predict_dir, 'STN_vector_img', 'images', str(patient_id), patient_tf, motion_name, 'pred_img.nii.gz')
    x = x_list[i]
    y = os.path.join(cg.data_dir, 'contour_dataset/processed_HR_data', str(patient_id), patient_tf,  'HR_ED_zoomed_crop_flip_clean.nii.gz')
